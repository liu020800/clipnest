...
- 自动记录文本剪贴板历史,复制文本后会写入本地 SQLite。
- 搜索窗口支持像 Windows `Win+V` 一样使用历史内容粘贴:选中条目后按 Enter 或双击,会写回剪贴板、回到之前窗口并模拟粘贴。
- 设置页新增"自动记录剪贴板"开关和"剪贴板历史保留数"设置。

### 变更

- 搜索窗口 Enter 行为从"复制并关闭"调整为"粘贴到原窗口";Ctrl+C 与详情页复制按钮仍保持只复制。
- 自动记录的剪贴板历史会打上 `剪贴板` 标签,重复内容不会新增多条,而是更新到最近。

### 修复

- 修复开机自启后透明 WebView 过早显示导致前端白屏的问题。
- 启动完成后保持托盘驻留,不再主动弹出搜索窗口。
- 修复开机自启指向调试版 exe 时导致安装版更新不生效的问题。
